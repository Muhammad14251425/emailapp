...
interface IEmail extends Document {
  ...
  subject: string;
  body: string;
  status: "send" | "failed"; // Status field included
...
const EmailSchema = new Schema<IEmail>(
  {
    ...
    status: { type: String, enum: ["send", "failed"], required: true }, // Status: "send" or "failed"
    attachments: [
      {
        filename: { type: String },
        content: { type: Schema.Types.Mixed }, // Supports both string & Buffer
        path: { type: String, required: true },
        contentType: { type: String },
        encoding: { type: String },
        headers: { type: Schema.Types.Mixed }, // Allows flexible key-value storage
        cid: { type: String }, // Content ID for inline images
      },
    ],
